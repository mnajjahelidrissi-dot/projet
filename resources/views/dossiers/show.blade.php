...
@section('page-actions')
    <a href="{{ route('dossiers.modifier', $dossier) }}" class="text-sm border border-gray-200 px-3 py-1.5 rounded-lg text-gray-600 hover:bg-gray-50">Modifier</a>
@endsection
...
<div class="bg-white rounded-xl border border-gray-100 p-5 mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        @if(auth()->user()->estAdministrateur() || auth()->user()->estResponsable())
            <div>
                <form method="POST" action="{{ route('dossiers.affecter-agent', $dossier) }}" class="flex flex-col sm:flex-row gap-3 items-start sm:items-end">
                    @csrf
                    <div class="flex-1 w-full">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Affecter un agent</label>
                        <select name="agent_id" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" required>
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div class="border border-gray-100 rounded-lg p-4 mb-3 last:mb-0">
                <div class="flex items-center justify-between">
                    ...
                    @if($demande->montant)
                    <span class="text-sm font-semibold text-gray-700">{{ number_format($demande->montant, 2) }} MAD</span>
                    @endif
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <form method="POST" action="{{ route('documents.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="text-xs font-medium text-gray-600 mb-1 block">Fichier (PDF / Image, max 10MB)</label>
                ...
                <button type="submit" class="mt-3 text-sm bg-[#0f2744] text-white px-4 py-1.5 rounded-lg hover:bg-[#1a4a8a]">
                    Archiver
                </button>
            </form>
            ...
            <div class="flex items-center justify-between py-2 border-t border-gray-50 first:border-t-0">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg {{ $doc->isPdf() ? 'bg-red-50' : 'bg-blue-50' }} flex items-center justify-center flex-shrink-0">
                        <svg class="w-4 h-4 {{ $doc->isPdf() ? 'text-red-500' : 'text-blue-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
